...
from tri_arb.cli.utils.validators import validate_symbol, validate_price, validate_quantity
...
def place(
    ...
    order_type: str = typer.Option(
        ...
        "--type",
        "-t",
        help="订单类型 (limit 或 market，默认 limit)"
    ),
    price: Optional[float] = typer.Option(
        None,
        "--price",
        "-p",
        help="限价单价格（market 订单不需要）"
    ),
    ...
    try:
        ...
        if exchange_type == ExchangeType.PERP:
            if position_side is None:
                raise ValueError("永续合约订单必须提供 --position-side 参数 (LONG 或 SHORT)")
...
def cancel(
    ...
    try:
        ...
        if result:
            ...
            console.print(f"[yellow]订单 {order_id} 取消失败（可能已完成或不存在）[/yellow]")
