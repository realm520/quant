...
class GatePerpExchange(BaseExchange):
    ...
    def _generate_signature(
        ...
        signature = hmac.new(
            self.api_secret.encode("utf-8"), payload.encode("utf-8"), hashlib.sha512
        ).hexdigest()

        logger.debug(
            "Gate signature details",
            method=method,
            path=url_path,
    ...
    async def _request(
        ...
        if method.upper() == "GET":
            ...
            response = await self.client.post(
                full_url, headers=headers, content=body_string
            )
        ...
        if response.status_code != 200:
            logger.error(
                "Gate API error", status=response.status_code, response=response.text
            )
    ...
    async def subscribe_ticker(self, symbol: str):
        ...
        raise NotImplementedError(
            "Gate.io WebSocket ticker subscription not implemented"
        )

    async def subscribe_orderbook(self, symbol: str):
        """订阅订单簿（WebSocket，占位符）."""
        raise NotImplementedError(
            "Gate.io WebSocket orderbook subscription not implemented"
        )
