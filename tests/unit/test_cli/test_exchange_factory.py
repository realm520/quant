...
class TestCreateExchangeSpot:
    ...
    def test_create_spot_with_explicit_credentials(self):
        ...
        exchange = create_exchange(
            ExchangeType.SPOT, api_key="test_key", api_secret="test_secret"
        )
    ...
    @patch.dict(
        os.environ, {"XT_API_KEY": "env_spot_key", "XT_API_SECRET": "env_spot_secret"}
    )
    ...
    def test_create_spot_explicit_overrides_environment(self):
        ...
        exchange = create_exchange(
            ExchangeType.SPOT, api_key="explicit_key", api_secret="explicit_secret"
        )
    ...
    def test_create_spot_missing_api_key(self):
        ...
        with pytest.raises(
            ValueError, match="现货交易需要配置 XT_API_KEY 和 XT_API_SECRET"
        ):
...
class TestCreateExchangePerp:
    ...
    def test_create_perp_with_explicit_credentials(self):
        ...
        exchange = create_exchange(
            ExchangeType.PERP, api_key="test_perp_key", api_secret="test_perp_secret"
        )
    ...
    @patch.dict(
        os.environ,
        {"XT_PERP_API_KEY": "env_perp_key", "XT_PERP_API_SECRET": "env_perp_secret"},
    )
    ...
    def test_create_perp_missing_api_key(self):
        ...
        with pytest.raises(
            ValueError,
            match="永续合约交易需要配置 XT_PERP_API_KEY 和 XT_PERP_API_SECRET",
        ):
...
class TestCreateExchangeSeparateCredentials:
    ...
    @patch.dict(os.environ, {"XT_API_KEY": "spot_key", "XT_API_SECRET": "spot_secret"})
