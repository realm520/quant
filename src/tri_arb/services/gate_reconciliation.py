...
class GateReconciliationService(BaseReconciliationService):
    ...
    def __init__(
        ...
        logger.info("Gate.io order data reconciliation service initialized (auto-discover symbols)")
    ...
    async def _discover_contracts(self, session: AsyncSession, since: datetime) -> List[str]:
        contracts: set[str] = set()

        order_stmt = select(GateOrder.contract).where(
            GateOrder.update_time >= since
        ).distinct()
        order_result = await session.execute(order_stmt)
        contracts.update(row[0] for row in order_result.fetchall() if row[0])

        trade_stmt = select(GateTrade.contract).where(
            GateTrade.create_time >= since
        ).distinct()
        ...
        try:
            ...
            position_contracts = {pos.get("contract") for pos in positions if pos.get("contract")}
        ...
        try:
            ...
            order_contracts = {order.get("contract") for order in open_orders if order.get("contract")}
            if order_contracts:
                logger.info(
    ...
    async def reconcile_orders(self, session: AsyncSession, start_time: datetime, end_time: datetime) -> Dict[str, int]:
        ...
        try:
            ...
            if not contracts:
                logger.debug("No Gate.io contracts discovered, skipping order reconciliation")
                return stats
                ...
                try:
                    ...
                    for order in orders:
                        try:
                            ...
                            existing_stmt = select(GateOrder.order_id, GateOrder.update_time).where(
                                GateOrder.order_id == order_record['order_id'],
                                GateOrder.update_time == order_record['update_time']
                            ).limit(1)
    ...
    def _convert_order_to_db_model(self, order: dict) -> dict:
        ...
        return {
            ...
            'update_time': safe_datetime(order.get('finish_time')) or datetime.utcnow(),  # Gate 没有单独的 update_time
