...
def format_balance_table(
    balances: Dict[str, Dict[str, Decimal]], exchange_instance
) -> None:
    ...
    """
    table = Table(
        title=f"Account Balance {exchange_instance.get_name()}",
        show_header=True,
    ...
    console.print(table)
    console.print(
        f"Data fetched at: {datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S UTC')}"
    )
    ...
    for pos in positions:
        ...
        if isinstance(pos, dict):
            ...
            if "instId" in pos:
                ...
                side = (
                    "Long" if pos.get("pos", Decimal("0")) > 0 else "Short"
            ...
            liquidation_price = (
                ...
                if hasattr(pos, "liquidation_price")
                else Decimal("0")
            )
...
def format_open_orders_table(orders: List[Dict[str, Any]], exchange) -> None:
    ...
    for order in orders:
        ...
        if "contract" in order and "instId" not in order:
            ...
            if order_time:
                time_str = datetime.fromtimestamp(order_time).strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
            else:
                time_str = "N/A"
        # Check if it's OKX format (has 'instId')
            ...
            if order_time_ms:
                time_str = datetime.fromtimestamp(order_time_ms / 1000).strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
            else:
                time_str = "N/A"
            ...
            if order_time:
                time_str = datetime.fromtimestamp(order_time / 1000).strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
            else:
                time_str = "N/A"
...
def format_orderbook_table(orderbook: Any, symbol: str, limit: int = 10) -> None:
    ...
    if orderbook.bids and orderbook.asks:
        ...
        spread_pct = (
            (spread / orderbook.bids[0][0] * 100)
            if orderbook.bids[0][0] > 0
            else Decimal("0")
        )
...
def format_order_summary(order: Any) -> None:
    ...
    fields = [
        ...
        (
            "Exchange ID",
            order.exchange_order_id if hasattr(order, "exchange_order_id") else "N/A",
        ),
        (
            "Symbol",
            order.symbol if hasattr(order, "symbol") else order.trading_pair.symbol,
        ),
        ...
        (
            "Created At",
            (
                order.created_at.strftime("%Y-%m-%d %H:%M:%S UTC")
