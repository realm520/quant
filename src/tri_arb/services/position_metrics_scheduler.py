...
class PositionMetricsScheduler:
    ...
    async def _init_contract_multiplier_service(self):
        ...
        try:
            ...
            if not config_path.exists():
                logger.warning(
                    f"配置文件不存在: {self.config_path}，将使用默认合约乘数"
                )
        ...
        try:
            ...
            for account_id, account_config in accounts.items():
                if not isinstance(account_config, dict):
                    logger.warning(
                        f"账号配置不是字典类型: {account_id} - {type(account_config)}"
                    )
                    continue
                ...
                if exchange_name not in ["binance", "xt"]:
                    logger.debug(
                        f"跳过账号 {account_id}: 交易所 {exchange_name} 不在支持列表中"
                    )
                    continue
            ...
            async with self.db_manager.session() as session:
                ...
                for exchange_name in ["binance", "xt"]:
                    ...
                    logger.info(
                        f"处理交易所: {exchange_name}, 账号数: {len(account_list)}"
                    )
                    ...
                    for account_config in account_list:
                        ...
                        try:
                            ...
                            contract_multiplier_getter: Optional[
                            ...
                            today_snapshot_result = await session.execute(
                                today_snapshot_query
                            )
                            ...
                            if not today_snapshot:
                                ...
                                today_snapshot = (
                                    today_snapshot_result.scalar_one_or_none()
                                )
                            ...
                            if today_snapshot:
                                ...
                                for snapshot in all_snapshots:
                                    today_initial_positions_dict[snapshot.symbol] = {
                                        ...
                                        "initial_long_qty": snapshot.left_long_qty
                                        or Decimal("0"),
                                        "initial_short_qty": snapshot.left_short_qty
                                        or Decimal("0"),
                                        "initial_long_value": snapshot.left_long_value
                                        or Decimal("0"),
                                        "initial_short_value": snapshot.left_short_value
                                        or Decimal("0"),
                                    }
                            ...
                            if today_daily_stats and today_date in today_daily_stats:
                                ...
                                for snapshot in all_snapshots:
                                    ...
                                    midnight_matched_qty = (
                                        snapshot.matched_qty or Decimal("0")
                                    )
                                    midnight_left_long_qty = (
                                        snapshot.left_long_qty or Decimal("0")
                                    )
                                    midnight_left_short_qty = (
                                        snapshot.left_short_qty or Decimal("0")
                                    )
                                    ...
                                    cumulative_buy_vol = (
                                        midnight_left_long_qty + midnight_matched_qty
                                    )
                                    cumulative_sell_vol = (
                                        midnight_left_short_qty + midnight_matched_qty
                                    )

                                    # 从零点快照读取累计市值和平均价格
                                    midnight_long_value = (
                                        snapshot.long_value or Decimal("0")
                                    )
                                    midnight_short_value = (
                                        snapshot.short_value or Decimal("0")
                                    )
                                    midnight_avg_buy_prz = (
                                        snapshot.avg_buy_prz or Decimal("0")
                                    )
                                    midnight_avg_sell_prz = (
                                        snapshot.avg_sell_prz or Decimal("0")
                                    )
                                ...
                                today_series_result = (
                                    calc._calc_daily_realized_series_with_initial(
                                        daily_stats={
                                            today_date: today_daily_stats.get(
                                                today_date, {}
                                            )
                                        },
                                ...
                                if today_date in today_series_result:
                                    for symbol, metrics in today_series_result[
                                        today_date
                                    ].items():
                                        # 获取收盘价
                                        close_prices = await calc._get_close_prices(
                                            start_time, end_time, symbol
                                        )
                                        close_prz = close_prices.get(
                                            symbol, Decimal("0")
                                        )

                                        # 计算未实现盈亏
                                        left_long_qty = metrics.get(
                                            "close_left_long_qty", Decimal("0")
                                        )
                                        left_short_qty = metrics.get(
                                            "close_left_short_qty", Decimal("0")
                                        )
                                        avg_buy_prz = metrics.get(
                                            "avg_buy_prz", Decimal("0")
                                        )
                                        avg_sell_prz = metrics.get(
                                            "avg_sell_prz", Decimal("0")
                                        )
                                        unrealized_pnl = Decimal("0")
                                        if close_prz > 0:
                                            unrealized_pnl = left_long_qty * (
                                                close_prz - avg_buy_prz
                                            ) + left_short_qty * (
                                                avg_sell_prz - close_prz
                                            )

                                        today_metrics[symbol] = {
                                            "buy_volume": metrics.get(
                                                "daily_buy_volume", Decimal("0")
                                            ),
                                            "sell_volume": metrics.get(
                                                "daily_sell_volume", Decimal("0")
                                            ),
                                            "buy_trade_value": metrics.get(
                                                "daily_buy_value", Decimal("0")
                                            ),
                                            "sell_trade_value": metrics.get(
                                                "daily_sell_value", Decimal("0")
                                            ),
                                            "long_qty": metrics.get(
                                                "total_long_qty", Decimal("0")
                                            ),
                                            "short_qty": metrics.get(
                                                "total_short_qty", Decimal("0")
                                            ),
                                            "long_value": metrics.get(
                                                "total_long_value", Decimal("0")
                                            ),
                                            "short_value": metrics.get(
                                                "total_short_value", Decimal("0")
                                            ),
                                            "avg_buy_prz": avg_buy_prz,
                                            "avg_sell_prz": avg_sell_prz,
                                            "matched_qty": metrics.get(
                                                "matched_qty", Decimal("0")
                                            ),
                                            "left_long_qty": left_long_qty,
                                            "left_short_qty": left_short_qty,
                                            "left_long_value": metrics.get(
                                                "close_left_long_value", Decimal("0")
                                            ),
                                            "left_short_value": metrics.get(
                                                "close_left_short_value", Decimal("0")
                                            ),
                                            "close_prz": close_prz,
                                            "unrealized_pnl": unrealized_pnl,
                                            "daily_realized_pnl": metrics.get(
                                                "daily_realized_pnl", Decimal("0")
                                            ),
                                            "cumulative_realized_pnl": metrics.get(
                            ...
                            symbol_count = len(
                                [k for k in today_metrics.keys() if k != "TOTAL"]
                            )
                            logger.info(
                                f"账号 {account_id} 找到 {symbol_count} 个交易对"
                            )

                            if symbol_count == 0:
                                logger.warning(
                                    f"账号 {account_id} 没有找到交易对数据，跳过"
                                )
                                continue
                            ...
                            for symbol_key, m in today_metrics.items():
                                ...
                                midnight_result = await session.execute(
                                    midnight_snapshot_query
                                )
                                ...
                                if midnight_snapshot:
                                    midnight_matched_qty = (
                                        midnight_snapshot.matched_qty or Decimal("0")
                                    )
                                    cumulative_realized_pnl_at_midnight = (
                                        midnight_snapshot.cumulative_realized_pnl
                                        or Decimal("0")
                                    )
                                else:
                                    logger.warning(
                                ...
                                today_unrealized_pnl = m.get(
                                    "unrealized_pnl", Decimal("0")
                                )
                                today_realized_pnl = m.get(
                                    "daily_realized_pnl", Decimal("0")
                                )
                                # 当前时刻的累积已实现盈亏 = 零点快照的累积已实现 + 今日新增的已实现
                                cumulative_realized_pnl_now = (
                                    cumulative_realized_pnl_at_midnight
                                    + today_realized_pnl
                                )
                                ...
                                cumulative_pnl = (
                                    cumulative_realized_pnl_now + today_unrealized_pnl
                                )

                                # 在控制台输出详细指标（使用表格格式）
                                logger.info(
                                    f"计算完成: {account_id} - {exchange_name} - {symbol_key}"
                                )
                                # 创建一个修改后的 today_m，使用正确的今日新增已实现盈亏和开盘初始仓位
                                today_m_corrected = m.copy()
                                today_m_corrected["realized_pnl"] = today_realized_pnl
                                today_m_corrected["daily_pnl"] = (
                                    today_realized_pnl + today_unrealized_pnl
                                )
                                today_m_corrected["cumulative_realized_pnl"] = (
                                    cumulative_realized_pnl_now  # 累计已实现盈亏
                                )
                                if midnight_snapshot:
                                    # 今日的初始仓位 = 今日零点快照的收盘仓位（昨日收盘）
                                    today_m_corrected["initial_long_qty"] = (
                                        midnight_snapshot.left_long_qty or Decimal("0")
                                    )
                                    today_m_corrected["initial_short_qty"] = (
                                        midnight_snapshot.left_short_qty or Decimal("0")
                                    )
                                    today_m_corrected["initial_long_value"] = (
                                        midnight_snapshot.left_long_value
                                        or Decimal("0")
                                    )
                                    today_m_corrected["initial_short_value"] = (
                                        midnight_snapshot.left_short_value
                                        or Decimal("0")
                                    )
                                ...
                                if midnight_snapshot:
                                    yesterday_m_for_display = {
                                        "left_long_qty": midnight_snapshot.left_long_qty
                                        or Decimal("0"),
                                        "left_short_qty": midnight_snapshot.left_short_qty
                                        or Decimal("0"),
                                        "left_long_value": midnight_snapshot.left_long_value
                                        or Decimal("0"),
                                        "left_short_value": midnight_snapshot.left_short_value
                                        or Decimal("0"),
                                        "avg_buy_prz": midnight_snapshot.avg_buy_prz
                                        or Decimal("0"),  # 昨日平均买价
                                        "avg_sell_prz": midnight_snapshot.avg_sell_prz
                                        or Decimal("0"),  # 昨日平均卖价
                                ...
                                open_left_long_qty_from_snapshot = (
                                    midnight_snapshot.left_long_qty
                                    if midnight_snapshot
                                    else Decimal("0")
                                )
                                open_left_short_qty_from_snapshot = (
                                    midnight_snapshot.left_short_qty
                                    if midnight_snapshot
                                    else Decimal("0")
                                )
                                open_left_long_value_from_snapshot = (
                                    midnight_snapshot.left_long_value
                                    if midnight_snapshot
                                    else Decimal("0")
                                )
                                open_left_short_value_from_snapshot = (
                                    midnight_snapshot.left_short_value
                                    if midnight_snapshot
                                    else Decimal("0")
                                )
                                ...
                                metrics_record = PositionMetrics(
                                    ...
                                    daily_sum_sell_qty=m.get(
                                        "sell_volume", Decimal("0")
                                    ),
                                    daily_sum_buy_value=m.get(
                                        "buy_trade_value", Decimal("0")
                                    ),
                                    daily_sum_sell_value=m.get(
                                    ...
                                    left_short_qty=m.get(
                                        "left_short_qty", Decimal("0")
                                    ),
                                    left_long_value=m.get(
                                        "left_long_value", Decimal("0")
                                    ),
                                    left_short_value=m.get(
                                ...
                                position_pre_long_qty.labels(**labels).set(
                                    float(open_left_long_qty_from_snapshot)
                                )
                                position_pre_short_qty.labels(**labels).set(
                                    float(open_left_short_qty_from_snapshot)
                                )
                                position_pre_long_value.labels(**labels).set(
                                    float(open_left_long_value_from_snapshot)
                                )
                                position_pre_short_value.labels(**labels).set(
                                    float(open_left_short_value_from_snapshot)
                                )

                                position_long_qty.labels(**labels).set(
                                    float(m.get("long_qty", Decimal("0")))
                                )
                                position_short_qty.labels(**labels).set(
                                    float(m.get("short_qty", Decimal("0")))
                                )
                                position_long_value.labels(**labels).set(
                                    float(m.get("long_value", Decimal("0")))
                                )
                                position_short_value.labels(**labels).set(
                                    float(m.get("short_value", Decimal("0")))
                                )
                                position_avg_buy_prz.labels(**labels).set(
                                    float(m.get("avg_buy_prz", Decimal("0")))
                                )
                                position_avg_sell_prz.labels(**labels).set(
                                    float(m.get("avg_sell_prz", Decimal("0")))
                                )

                                position_matched_qty.labels(**labels).set(
                                    float(m.get("matched_qty", Decimal("0")))
                                )
                                # 使用今日新增的已实现盈亏（而不是包含初始持仓的已实现盈亏）
                                position_realized_pnl.labels(**labels).set(
                                    float(today_realized_pnl)
                                )

                                position_left_long_qty.labels(**labels).set(
                                    float(m.get("left_long_qty", Decimal("0")))
                                )
                                position_left_short_qty.labels(**labels).set(
                                    float(m.get("left_short_qty", Decimal("0")))
                                )
                                position_left_long_value.labels(**labels).set(
                                    float(m.get("left_long_value", Decimal("0")))
                                )
                                position_left_short_value.labels(**labels).set(
                                    float(m.get("left_short_value", Decimal("0")))
                                )
                                position_close_prz.labels(**labels).set(
                                    float(m.get("close_prz", Decimal("0")))
                                )
                                position_unrealized_pnl.labels(**labels).set(
                                    float(m.get("unrealized_pnl", Decimal("0")))
                                )

                                # 使用正确的单日 PnL（今日新增已实现 + 今日未实现）
                                position_daily_pnl.labels(**labels).set(
                                    float(today_realized_pnl + today_unrealized_pnl)
                                )
                                position_cumulative_pnl.labels(**labels).set(
    ...
    async def _rebuild_midnight_snapshots(
        ...
        try:
            ...
            if calc.exchange == "binance":
                earliest_query = earliest_query.where(
                    calc.TradeModel.exchange == "binance_perp"
                )
            ...
            if calc.account_id:
                earliest_query = earliest_query.where(
                    calc.TradeModel.account_id == calc.account_id
                )
                latest_query = latest_query.where(
                    calc.TradeModel.account_id == calc.account_id
                )
            ...
            for trade_date, day_data in sorted(daily_series.items()):
                ...
                for sym, metrics in day_data.items():
                    ...
                    stmt = pg_insert(PositionMetrics).values(
                        ...
                        symbol=sym,
                        # 1. 开盘持仓（昨收持仓）
                        open_left_long_qty=metrics.get(
                            "open_left_long_qty", Decimal("0")
                        ),
                        open_left_short_qty=metrics.get(
                            "open_left_short_qty", Decimal("0")
                        ),
                        open_left_long_value=metrics.get(
                            "open_left_long_value", Decimal("0")
                        ),
                        open_left_short_value=metrics.get(
                            "open_left_short_value", Decimal("0")
                        ),
                        # 2. 当日成交量
                        daily_sum_buy_qty=metrics.get("daily_buy_volume", Decimal("0")),
                        daily_sum_sell_qty=metrics.get(
                            "daily_sell_volume", Decimal("0")
                        ),
                        daily_sum_buy_value=metrics.get(
                            "daily_buy_value", Decimal("0")
                        ),
                        daily_sum_sell_value=metrics.get(
                            "daily_sell_value", Decimal("0")
                        ),
                        # 3. 总持仓（初始持仓 + 当日成交量）
                        ...
                        left_long_qty=metrics.get(
                            "close_left_long_qty", Decimal("0")
                        ),  # 从 calc_daily_realized_series 返回的 close_left_* key 获取
                        left_short_qty=metrics.get(
                            "close_left_short_qty", Decimal("0")
                        ),
                        left_long_value=metrics.get(
    ...
    async def _create_empty_midnight_snapshot(
        ...
        try:
            ...
            symbols_query = select(TradeModel.symbol.distinct()).where(
                cast(time_column, Date) == today_date
            )

            if exchange == "binance":
                symbols_query = symbols_query.where(
                    TradeModel.exchange == "binance_perp"
                )
            if account_id:
    ...
    async def _get_initial_positions_from_trades(
        ...
        try:
            ...
            query = (
                select(
                    TradeModel.side, TradeModel.price, TradeModel.quantity, time_column
                )
    ...
    def _log_metrics_table(
        ...
        yesterday_cumulative_realized_pnl = yesterday_m.get(
            "cumulative_realized_pnl", Decimal("0")
        )
        ...
        table.add_row(
            ...
            _format_decimal(yesterday_left_long_qty, 2),
        )
        table.add_row(
            "  昨日空头持仓量 (pre_short_qty)",
            _format_decimal(yesterday_left_short_qty, 2),
        )
        table.add_row(
            "  昨日多头市值 (pre_long_value)",
            _format_decimal(yesterday_left_long_value, 4),
        )
        table.add_row(
            "  昨日空头市值 (pre_short_value)",
            _format_decimal(yesterday_left_short_value, 4),
        )
        if yesterday_avg_buy_prz > 0:
            table.add_row(
                "  昨日平均买价 (昨日avg_buy_prz)",
                _format_decimal(yesterday_avg_buy_prz, 8),
            )
        if yesterday_avg_sell_prz > 0:
            table.add_row(
                "  昨日平均卖价 (昨日avg_sell_prz)",
                _format_decimal(yesterday_avg_sell_prz, 8),
            )
        table.add_row("", "")
