...
async def example_get_all_positions():
    ...
    # 创建交易所实例
    exchange = BinancePerpExchange(api_key=api_key, api_secret=api_secret)
...
async def example_compare_positions():
    ...
    try:
        ...
        for pos in positions:
            ...
            # 计算保证金：名义价值 / 杠杆
            margin = (
                notional / leverage if leverage > 0 and notional > 0 else Decimal("0")
            )
            ...
            if position_side == "LONG" or (
                position_side == "BOTH" and pos["positionAmt"] > 0
            ):
                long_positions.append(pos)
            elif position_side == "SHORT" or (
                position_side == "BOTH" and pos["positionAmt"] < 0
            ):
        ...
        print("\n💰 盈利前3:")
        profitable = sorted(
            positions, key=lambda x: x["unRealizedProfit"], reverse=True
        )[:3]
