...
class BinanceReconciliationService(BaseReconciliationService):
    ...
    def __init__(
        ...
        logger.info(
            "Binance reconciliation service initialized (auto-discover symbols)"
        )
    ...
    async def reconcile_orders(
        ...
        try:
            ...
            if open_orders:
                table = Table(
                    title="Binance 当前挂单 (前5条)", show_edge=False, box=None
                )
            ...
            db_open_orders_dict = {
                order.order_id: order for order in db_result.scalars()
            }
            ...
            if missing_order_ids:
                logger.debug(
                    f"Found {len(missing_order_ids)} orders in DB but not in API, querying history"
                )
                        ...
                        try:
                            ...
                            for historical_order in historical_orders:
                                if historical_order["orderId"] in order_ids:
                                    order_record = self._convert_order_to_db_model(
                                        historical_order
                                    )
            ...
            for symbol in symbols:
                ...
                async with session.begin_nested():
                    try:
                        ...
                        if trades:
                            trade_table = Table(
                                title=f"Binance {symbol} 成交对账 (前5条)",
                                show_edge=False,
                                box=None,
                            )
    ...
    def _convert_order_to_db_model(self, order: dict) -> dict:
        ...
        return {
            ...
            "original_price": (
                Decimal(str(order["price"])) if order.get("price") else None
            ),
            "average_price": (
                Decimal(str(order["avgPrice"]))
    ...
    async def _get_active_symbols(
        ...
        try:
            ...
            if position_symbols:
                ...
                pos_table = Table(
                    title="Binance 持仓发现的交易对", show_edge=False, box=None
                )
