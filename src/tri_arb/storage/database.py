...
class DatabaseManager:
    ...
    async def create_tables(self):
        ...
        async with self.async_engine.connect() as conn:
            ...
            def _create_safe(sync_conn, metadata, name: str):
                ...
                for table_name in sorted(missing_tables):
                    ...
                    try:
                        ...
                        if not table_exists:
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    logger.debug(
                                        f"Generated SQL for {table_name} (length: {len(create_table_sql)})"
                                    )
                                ...
                                try:
                                    ...
                                    logger.info(
                                        f"Executing CREATE TABLE for {table_name}..."
                                    )
                                    ...
                                    try:
                                        ...
                                        sync_conn.commit()
                                        logger.info(
                                            f"✓ CREATE TABLE executed and committed for {table_name}"
                                        )
                                        ...
                                        if (
                                            ...
                                            and "constraint" in error_str
                                        ):
                                            logger.warning(
                                                f"Constraint already exists for {table_name}, attempting to drop and recreate..."
                                            )
                                            # 提取约束名
                                            import re
                                            ...
                                            if constraint_match:
                                                ...
                                                logger.info(
                                                    f"Dropping existing constraint: {constraint_name}"
                                                )
                                                # 尝试删除约束（可能约束在另一个表上）
                                                try:
                                                    # 查找约束所属的表
                                                    ...
                                                    if result:
                                                        ...
                                                        cur.execute(
                                                            f'ALTER TABLE "{constraint_table}" DROP CONSTRAINT IF EXISTS "{constraint_name}"'
                                                        )
                                                        cur.close()
                                                        sync_conn.commit()
                                                        logger.info(
                                                            f"✓ Dropped constraint {constraint_name} from {constraint_table}"
                                                        )
                                                        ...
                                                        sync_conn.commit()
                                                        logger.info(
                                                            f"✓ CREATE TABLE executed and committed for {table_name} (after dropping constraint)"
                                                        )
                                                    else:
                                                        raise create_err
                                                except Exception as drop_err:
                                                    logger.error(
                                                        f"Failed to drop constraint: {drop_err}"
                                                    )
                                                    raise create_err
                                            else:
                                                raise create_err
                                    ...
                                    logger.info(
                                        f"Verification: table {table_name} exists = {table_exists_after}"
                                    )
                                ...
                                if (
                                    "already exists" in create_err_str
                                    or "duplicate" in create_err_str
                                ):
                                    # 表已存在，验证一下
                                    inspector = inspect(sync_conn)
                                    if inspector.has_table(table_name):
                                    ...
                                    logger.error(
                                        f"Failed to create table {table_name}: {create_err}"
                                    )
                        ...
                        if table_created:
                            for index in table.indexes:
                                try:
                                    ...
                                    if (
                                        "already exists" in idx_err_str
                                        or "duplicate" in idx_err_str
                                    ):
                                        logger.debug(
                                            f"Index {index.name} already exists (skipping)"
                                        )
                                    else:
                                        # 索引创建失败不影响表的使用，只记录警告
                                        logger.warning(
                                            f"Failed to create index {index.name}: {idx_err}"
                                        )
                        ...
                        if "already exists" in pe_str or "duplicate" in pe_str:
                            logger.debug(
                                f"Table {table_name} or its indexes already exist (skipping)"
                            )
                        ...
                        if "already exists" in error_str or "duplicate" in error_str:
                            ...
                            logger.error(
                                f"Unexpected error creating table {table_name}: {e}",
                                exc_info=True,
                            )
                ...
                for table_name in metadata_tables:
                    if table_name in existing_tables_after:
                        ...
                        logger.warning(
                            f"✗ Table missing after creation attempt: {table_name}"
                        )
                        # 如果表应该在 created_tables 中但实际不存在，从 created_tables 中移除
                        if table_name in created_tables:
                            logger.warning(
                                f"  Table {table_name} was in created_tables but does not exist in database"
                            )
                        ...
                        if table_name not in [t[0] for t in failed_tables]:
                            ...
                            logger.error(
                                f"  Adding to failed_tables: {table_name} - {error_msg}"
                            )
                            failed_tables.append((table_name, error_msg))
                ...
                if failed_tables:
                    ...
                    full_error_msg = (
                        f"Failed to create {len(failed_tables)} {name} table(s):\n"
                ...
                if missing_after_creation:
                    ...
                    logger.error(error_msg)
                    raise RuntimeError(
                        f"Tables missing after creation: {', '.join(missing_after_creation)}"
                    )
            ...
            try:
                ...
                if "already exists" in error_str or "duplicate" in error_str:
                    logger.warning(
                        f"Some Binance tables/indexes already exist (this is OK): {e}"
                    )
                else:
            ...
            try:
                ...
                if "already exists" in error_str or "duplicate" in error_str:
                    logger.warning(
                        f"Some OKX tables/indexes already exist (this is OK): {e}"
                    )
                else:
            ...
            try:
                ...
                if "already exists" in error_str or "duplicate" in error_str:
                    logger.warning(
                        f"Some Gate.io tables/indexes already exist (this is OK): {e}"
                    )
                else:
            ...
            try:
                ...
                if "already exists" in error_str or "duplicate" in error_str:
                    logger.warning(
                        f"Some XT WebSocket tables/indexes already exist (this is OK): {e}"
                    )
                else:
                    logger.error(
                        f"Failed to create XT WebSocket tables: {e}", exc_info=True
                    )
                    raise
            ...
            try:
                ...
                if "already exists" in error_str or "duplicate" in error_str:
                    logger.warning(
                        f"Some REST API tables/indexes already exist (this is OK): {e}"
                    )
                else:
                    logger.error(
                        f"Failed to create REST API tables: {e}", exc_info=True
                    )
                    raise
            ...
            try:
                ...
                if "already exists" in error_str or "duplicate" in error_str:
                    logger.warning(
                        f"Some XT REST API tables/indexes already exist (this is OK): {e}"
                    )
                else:
                    logger.error(
                        f"Failed to create XT REST API tables: {e}", exc_info=True
                    )
                    raise
            ...
            try:
                ...
                if "already exists" in error_str or "duplicate" in error_str:
                    logger.warning(
                        f"Some Exchange-specific REST API tables/indexes already exist (this is OK): {e}"
                    )
                else:
                    logger.error(
                        f"Failed to create Exchange-specific REST API tables: {e}",
                        exc_info=True,
                    )
                    raise
            ...
            try:
                ...
                if "already exists" in error_str or "duplicate" in error_str:
                    logger.warning(
                        f"Some Position Metrics tables/indexes already exist (this is OK): {e}"
                    )
                else:
                    logger.error(
                        f"Failed to create Position Metrics tables: {e}", exc_info=True
                    )
                    raise

        logger.info(
            "✅ All database tables created/verified (Binance + OKX + Gate.io + XT WebSocket + REST API + XT REST API + Exchange-specific REST API + Position Metrics)"
        )
    ...
    async def drop_tables(self):
        ...
        logger.warning(
            "Database tables dropped (Binance + OKX + Gate.io + XT WebSocket + REST API + XT REST API + Exchange-specific REST API + Position Metrics)"
        )
