...
def verify_row(row_data):
    ...
    if long_qty > 0:
        ...
        if diff > Decimal("0.00000001"):
            errors.append(
                f"avg_buy_prz 计算错误: 期望 {expected_avg_buy_prz:.10f}, 实际 {avg_buy_prz:.10f}, 差异 {diff:.10f}"
            )
        else:
            print(
                f"✓ avg_buy_prz = {avg_buy_prz:.10f} (long_value / long_qty = {long_value} / {long_qty})"
            )
    else:
        if avg_buy_prz != 0:
    ...
    if short_qty > 0:
        ...
        if diff > Decimal("0.00000001"):
            errors.append(
                f"avg_sell_prz 计算错误: 期望 {expected_avg_sell_prz:.10f}, 实际 {avg_sell_prz:.10f}, 差异 {diff:.10f}"
            )
        else:
            print(
                f"✓ avg_sell_prz = {avg_sell_prz:.10f} (short_value / short_qty = {short_value} / {short_qty})"
            )
    else:
        if avg_sell_prz != 0:
    ...
    if matched_qty != expected_matched_qty:
        errors.append(
            f"matched_qty 计算错误: 期望 {expected_matched_qty}, 实际 {matched_qty}"
        )
    else:
    ...
    if matched_qty > 0 and avg_sell_prz > 0 and avg_buy_prz > 0:
        ...
        if diff > Decimal("0.0001"):
            errors.append(
                f"daily_realized_pnl 计算错误: 期望 {expected_daily_realized_pnl:.10f}, 实际 {daily_realized_pnl:.10f}, 差异 {diff:.10f}"
            )
        else:
            print(
                f"✓ daily_realized_pnl = {daily_realized_pnl:.10f} (matched_qty * (avg_sell_prz - avg_buy_prz) = {matched_qty} * ({avg_sell_prz} - {avg_buy_prz}))"
            )
    ...
    if abs(left_long_qty - expected_left_long_qty) > Decimal("0.0001"):
        errors.append(
            f"left_long_qty 计算错误: 期望 {expected_left_long_qty}, 实际 {left_long_qty}"
        )
    else:
        print(
            f"✓ left_long_qty = {left_long_qty} (long_qty - matched_qty = {long_qty} - {matched_qty})"
        )
    ...
    if abs(left_short_qty - expected_left_short_qty) > Decimal("0.0001"):
        errors.append(
            f"left_short_qty 计算错误: 期望 {expected_left_short_qty}, 实际 {left_short_qty}"
        )
    else:
        print(
            f"✓ left_short_qty = {left_short_qty} (short_qty - matched_qty = {short_qty} - {matched_qty})"
        )
    ...
    if left_long_qty > 0:
        ...
        if diff > Decimal("0.0001"):
            errors.append(
                f"left_long_value 计算错误: 期望 {expected_left_long_value:.10f}, 实际 {left_long_value:.10f}, 差异 {diff:.10f}"
            )
        else:
            print(
                f"✓ left_long_value = {left_long_value:.10f} (left_long_qty * avg_buy_prz = {left_long_qty} * {avg_buy_prz})"
            )
    else:
    ...
    if left_short_qty > 0:
        ...
        if diff > Decimal("0.0001"):
            errors.append(
                f"left_short_value 计算错误: 期望 {expected_left_short_value:.10f}, 实际 {left_short_value:.10f}, 差异 {diff:.10f}"
            )
        else:
            print(
                f"✓ left_short_value = {left_short_value:.10f} (left_short_qty * avg_sell_prz = {left_short_qty} * {avg_sell_prz})"
            )
    else:
    ...
    if close_prz > 0:
        expected_unrealized_pnl = left_long_qty * (
            close_prz - avg_buy_prz
        ) + left_short_qty * (avg_sell_prz - close_prz)
        diff = abs(expected_unrealized_pnl - unrealized_pnl)
        if diff > Decimal("0.0001"):
            errors.append(
                f"unrealized_pnl 计算错误: 期望 {expected_unrealized_pnl:.10f}, 实际 {unrealized_pnl:.10f}, 差异 {diff:.10f}"
            )
            print(f"详细计算:")
            print(
                f"  left_long_qty * (close_prz - avg_buy_prz) = {left_long_qty} * ({close_prz} - {avg_buy_prz}) = {left_long_qty * (close_prz - avg_buy_prz):.10f}"
            )
            print(
                f"  left_short_qty * (avg_sell_prz - close_prz) = {left_short_qty} * ({avg_sell_prz} - {close_prz}) = {left_short_qty * (avg_sell_prz - close_prz):.10f}"
            )
    ...
    if diff > Decimal("0.0001"):
        errors.append(
            f"daily_pnl 计算错误: 期望 {expected_daily_pnl:.10f}, 实际 {daily_pnl:.10f}, 差异 {diff:.10f}"
        )
    else:
        print(
            f"✓ daily_pnl = {daily_pnl:.10f} (daily_realized_pnl + unrealized_pnl = {daily_realized_pnl} + {unrealized_pnl})"
        )
    ...
    if diff > Decimal("0.0001"):
        errors.append(
            f"cumulative_pnl 计算错误: 期望 {expected_cumulative_pnl:.10f}, 实际 {cumulative_pnl:.10f}, 差异 {diff:.10f}"
        )
    else:
        print(
            f"✓ cumulative_pnl = {cumulative_pnl:.10f} (cumulative_realized_pnl + unrealized_pnl = {cumulative_realized_pnl} + {unrealized_pnl})"
        )
