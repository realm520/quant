    ...
    async with db_manager.session() as session:
        try:
            ...
            def sync_getter(symbol: str) -> Decimal:
                """同步获取合约乘数."""
                try:
                    return contract_multiplier_service.get_multiplier_sync(
                        exchange, symbol
                    )
                except Exception:
                    return Decimal("1")
            ...
            existing_times_query = (
                ...
                .where(PositionMetrics.timestamp >= start_time)
                .where(PositionMetrics.timestamp <= end_time)
            )
            if symbol:
...
async def main():
    parser = argparse.ArgumentParser(
        description="补充指定时间段的 position_metrics 数据"
    )
    ...
    parser.add_argument(
        "--start-time", required=True, help="开始时间 (格式: YYYY-MM-DD HH:MM:SS)"
    )
    parser.add_argument(
        "--end-time", required=True, help="结束时间 (格式: YYYY-MM-DD HH:MM:SS)"
    )
    parser.add_argument(
        "--database-url",
        default=None,
