...
class XTRestDataService:
    ...
    async def save_spot_balance(
        self, balances_data: Dict[str, Any], query_type: str = "scheduled"
    ):
        """保存XT现货账户余额到数据库.
            ...
            async with self.db_manager.session() as session:
                for asset, data in balances_data.items():
                    ...
                    balance_record = XTSpotBalance(
                        ...
                        raw_data=json.dumps(
                            raw_data_dict, ensure_ascii=False, default=str
                        ),
                    )
                ...
                logger.info(
                    f"Saved {len(balances_data)} XT spot balance records{account_info}"
                )
            ...
            async with self.db_manager.session() as session:
                for asset, data in balances_data.items():
                    ...
                    balance_record = XTPerpBalance(
                        ...
                        total=Decimal(str(data.get("total", 0))),
                        unrealized_pnl=(
                            Decimal(str(data.get("unrealized_pnl", 0)))
                            if data.get("unrealized_pnl") is not None
                            else None
                        ),
                        realized_pnl=(
                            Decimal(str(data.get("realized_pnl", 0)))
                            if data.get("realized_pnl") is not None
                            else None
                        ),
                        equity=(
                            Decimal(str(data.get("equity", 0)))
                            if data.get("equity") is not None
                            else None
                        ),
                        margin=(
                            Decimal(str(data.get("margin", 0)))
                            if data.get("margin") is not None
                            else None
                        ),
                        margin_ratio=(
                            Decimal(str(data.get("margin_ratio", 0)))
                            if data.get("margin_ratio") is not None
                            else None
                        ),
                        raw_data=json.dumps(
                            raw_data_dict, ensure_ascii=False, default=str
                ...
                logger.info(
                    f"Saved {len(balances_data)} XT perp balance records{account_info}"
                )
    ...
    async def save_perp_positions(
        self, positions_data: List[Dict[str, Any]], query_type: str = "scheduled"
    ):
        """保存XT合约账户仓位到数据库.
            ...
            async with self.db_manager.session() as session:
                ...
                for pos_data in positions_data:
                    ...
                    position_record = XTPerpPosition(
                        ...
                        maintenance_margin=(
                            Decimal(str(maint_margin)) if maint_margin else None
                        ),
                        raw_data=raw_data,
                    )
                ...
                logger.info(
                    f"Saved {len(positions_data)} XT perp position records{account_info}"
                )
        ...
        try:
            async with self.db_manager.session() as session:
                ...
                for pos_data in positions_data:
                    ...
                    position_amount = (
                        pos_data.get("positionSize")
                        or pos_data.get("positionAmt")
                        or pos_data.get("position_amount", "0")
                    )
                    entry_price = pos_data.get("entryPrice")
                    mark_price = pos_data.get("calMarkPrice") or pos_data.get(
                        "markPrice"
                ...
                logger.info(
                    f"Saved {len(positions_data)} XT rest position update records{account_info}"
                )
