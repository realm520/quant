...
async def _run_xt_watch_positions_async(
    ...
    account_label = (
        f"{account_id} ({account_name})" if account_name else account_id or "默认账号"
    )
    ...
    if symbol:
        normalized_target = (
            symbol.replace("/", "").replace("_", "").replace("-", "").upper()
        )
    ...
    async def fetch_positions(iteration_num: int):
        ...
        for pos in positions:
            try:
                if hasattr(pos, "symbol"):
                    ...
                    quantity = Decimal(
                        str(pos.get("positionSize") or pos.get("positionAmt") or "0")
                    )
                    entry_price = Decimal(str(pos.get("entryPrice") or "0"))
                    mark_price = Decimal(
                        str(pos.get("calMarkPrice") or pos.get("markPrice") or "0")
                    )
                    ...
                    realized_pnl = Decimal(
                        str(pos.get("realizedProfit") or pos.get("realizedPnl") or "0")
                    )
                    margin = Decimal(
                        str(pos.get("isolatedMargin") or pos.get("margin") or "0")
                    )
                    leverage = pos.get("leverage", "")
                    liquidation_price = Decimal(
                        str(pos.get("breakPrice") or pos.get("liquidationPrice") or "0")
                    )
                ...
                logger.warning(
                    f"账号 {account_label} 解析仓位记录失败", error=str(inner_exc)
                )
                if debug:
                    console.print_exception()
        ...
        if rows_added == 0:
            if symbol:
                console.print(
                    f"[yellow][账号 {account_label}] 未发现 {symbol} 的持仓[/yellow]"
                )
            else:
                console.print(f"[yellow][账号 {account_label}] 当前无持仓[/yellow]")
            return
        ...
        console.print(
            f"[green]✓[/green] [账号 {account_label}] 仓位数据 (perp) 已保存到 [cyan]xt_position_snapshot[/cyan]\n"
        )
...
def _run_xt_watch_positions(
    ...
    async def fetch_positions(iteration_num: int):
        ...
        console.print(position_table)
        console.print(
            f"[dim]数据获取时间: {datetime.datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S UTC')}[/dim]"
        )
        ...
        console.print(
            f"[green]✓[/green] [账号 {acc_label}] 仓位数据 (perp) 已保存到 [cyan]xt_position_snapshot[/cyan]\n"
        )
...
def _sign_payload(
    base: dict[str, Any], secret: str, use_milliseconds: bool
) -> dict[str, Any]:
    """为 Lark 请求生成签名。"""
...
async def _send_lark_text(
    ...
    if secret:
        payload_attempts.append(
            _sign_payload(base_body, secret, use_milliseconds=False)
        )
    ...
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            for attempt_idx, payload in enumerate(payload_attempts):
                ...
                if should_retry:
                    console.print(
                        "[yellow]Lark 签名校验失败，尝试使用毫秒时间戳重试...[/yellow]"
                    )
                    continue
    ...
    try:
        ...
        for pos in selected:
            try:
                ...
                mark_price = Decimal(
                    pos.get("calMarkPrice") or pos.get("markPrice") or "0"
                )
                liquidation_price = Decimal(
                    pos.get("breakPrice") or pos.get("liquidationPrice") or "0"
                )
                unrealized = Decimal(
                    pos.get("floatingPL") or pos.get("unRealizedProfit") or "0"
                )
                ...
                if liquidation_price and liquidation_price != Decimal("0"):
                    distance_pct = (
                        (mark_price - liquidation_price) / liquidation_price
                    ) * Decimal("100")
                else:
...
async def _evaluate_risk_ratio(
    ...
    async with db_manager.session() as session:
        ...
        margin_total = (
            Decimal(str(balance_row[1])) if balance_row[1] is not None else Decimal("0")
        )
    ...
    for (
        ...
        unrealized = (
            Decimal(str(unrealized_val)) if unrealized_val is not None else None
        )
...
def balance(
    ...
    passphrase: Optional[str] = typer.Option(
        None, "--passphrase", help="OKX 交易所需要的 passphrase（覆盖环境变量）"
    ),
    ...
    lark_webhook: Optional[str] = typer.Option(
        None, "--lark-webhook", help="Lark群机器人Webhook URL，用于推送仓位告警"
    ),
    lark_secret: Optional[str] = typer.Option(
        None, "--lark-secret", help="Lark机器人签名密钥（若启用安全校验需提供）"
    ),
    enable_lark: bool = typer.Option(
        False, "--enable-lark", help="是否启用 Lark 告警推送（默认 False）"
    ),
):
    """查询账户余额.
    ...
    try:
        ...
        exchange_instance = create_exchange(
            exchange_type, api_key, api_secret, exchange, passphrase=passphrase
        )
...
def positions(
    ...
    try:
        ...
        if symbol:
            ...
            normalized_symbol = (
                symbol.replace("/", "").replace("-", "").replace("_", "").upper()
            )
            ...
            for pos in positions_list:
                if isinstance(pos, dict):
                    ...
                    if "instId" in pos:
                        ...
                        side = (
                            "Long"
                            if pos.get("positionAmt", Decimal("0")) > 0
                            else "Short"
                        )
                        ...
                        margin = (
                            notional / leverage_num
                            if leverage_num > 0 and notional > 0
                            else Decimal("0")
                        )
                        roe = (
                    ...
                    roe = (
                        (pos.unrealized_pnl / pos.margin * 100)
                        if hasattr(pos, "margin") and pos.margin > 0
                        else Decimal("0")
                    )
                    csv_data.append(
                        {
                            ...
                            "roe": str(roe),
                            "leverage": str(
                                pos.leverage if hasattr(pos, "leverage") else 0
                            ),
                        }
                    )
            print_csv(csv_data)
...
def watch_balance(
    ...
    interval: int = typer.Option(
        5, "--interval", "-i", help="查询间隔（分钟），默认5分钟"
    ),
    api_key: Optional[str] = typer.Option(
        None, "--api-key", help="API 密钥（覆盖环境变量）"
    ),
    ...
    output: str = typer.Option(
        "table", "--output", "-o", help="输出格式 (table, json)"
    ),
    debug: bool = typer.Option(False, "--debug", help="启用调试模式"),
    ...
    try:
        ...
        def resolve_credentials(
            ...
            if target_exchange == ExchangeName.OKX:
                final_passphrase = passphrase_value or os.getenv(
        ...
        if accounts or all_accounts:
            ...
            if accounts:
                requested_ids = [
                    acc_id.strip() for acc_id in accounts.split(",") if acc_id.strip()
                ]
            else:
                requested_ids = [
                    acc.account_id for acc in account_manager.get_enabled_accounts()
                ]
            ...
            for acc_id in requested_ids:
                ...
                if not acc_config:
                    console.print(
                        f"[yellow]警告:[/yellow] 配置文件中未找到账号: {acc_id}，跳过"
                    )
                    continue
                if not acc_config.enabled:
                    console.print(
                        f"[yellow]警告:[/yellow] 账号 {acc_id} 未启用（enabled: false），跳过"
                    )
                    continue
            ...
            console.print(
                f"[cyan]多账号监控模式（{len(account_configs)} 个账号，配置总数 {total_accounts}）[/cyan]"
            )
            ...
            async def run_multi_account_watch():
                ...
                for acc in account_configs:
                    try:
                        ...
                        console.print(
                            f"[yellow]警告:[/yellow] 账号 {acc.account_id} 使用未支持的交易所 {acc.exchange}，跳过"
                        )
                        continue
                ...
                if not tasks:
                    console.print(
                        "[red]错误:[/red] 选择的账号交易所暂未支持 watch-balance 功能"
                    )
                    return
            ...
            try:
                ...
                if account_config:
                    ...
                    if not account_config.enabled:
                        console.print(
                            f"[yellow]警告:[/yellow] 账号 {account_id} 未启用（enabled: false）"
                        )
                    try:
                        exchange = ExchangeName(account_config.exchange.lower())
                    except ValueError:
                        console.print(
                            f"[red]错误:[/red] 账号 {account_id} 使用未支持的交易所 {account_config.exchange}"
                        )
                        raise typer.Exit(code=1)
                    if not api_key:
                    ...
                    database_url = account_manager.global_settings.get("database_url")
                    console.print(
                        f"[cyan]从配置文件加载账号: {account_id} ({account_config.name})[/cyan]"
                    )
                else:
                    console.print(
                        f"[yellow]警告:[/yellow] 配置文件中未找到账号 {account_id}，使用命令行参数或环境变量"
                    )
            except Exception as exc:
                console.print(
                    f"[yellow]警告:[/yellow] 读取配置文件失败: {exc}，使用命令行参数或环境变量"
                )

        final_api_key, final_api_secret, final_passphrase = resolve_credentials(
            exchange, api_key, api_secret, passphrase
        )
        if not final_api_key or not final_api_secret:
            console.print(
                f"[red]错误:[/red] 缺少 {exchange.value.upper()} API 密钥配置"
            )
            console.print("\n请设置环境变量或使用命令行参数:")
            console.print(
                f"  环境变量: export {exchange.value.upper()}_API_KEY=your_key && export {exchange.value.upper()}_API_SECRET=your_secret"
            )
            console.print("  命令行:   --api-key YOUR_KEY --api-secret YOUR_SECRET")
            console.print(
                "  配置文件: --config config/accounts.json --account-id <account_id>"
            )
            raise typer.Exit(code=1)

        account_label = (
            selected_account_config.name if selected_account_config else None
        )
        account_ref = (
            selected_account_config.account_id
            if selected_account_config
            else account_id
        )
...
async def _run_xt_watch_balance_async(
    ...
    exchange_instance = create_exchange(
        exchange_type, api_key, api_secret, ExchangeName.XT
    )
    db_manager = DatabaseManager(database_url=database_url)
    metrics_account = account_id or (account_name or "default")
    exchange_label = ExchangeName.XT.value
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    if not balance_data:
                        console.print(
                            f"[yellow][账号 {account_label}] 账户余额为空或所有币种余额为0[/yellow]"
                        )
                    else:
                        # 根据输出格式显示
                        if output == "json":
                        ...
                        try:
                            ...
                            for currency, data in balance_data.items():
                                ...
                                async with db_manager.session() as session:
                                    ...
                                    logger.info(
                                        f"账号 {account_label} 余额已保存到数据库: {currency.upper()}"
                                    )
                                    console.print(
                                        f"[green]✓[/green] [账号 {account_label}] 余额数据 ({exchange_type.value}) 已保存到 [cyan]xt_account_update[/cyan]: {currency.upper()}"
                                    )
                        except Exception as save_exc:
                            logger.warning(
                                f"账号 {account_label} 保存余额到数据库失败: {save_exc}"
                            )
                            console.print(
                                f"[red]✗[/red] [账号 {account_label}] 保存余额失败: {save_exc}"
                            )

                        next_query_time = datetime.datetime.now() + datetime.timedelta(
                            minutes=interval
                        )
                        console.print(
                            f"[dim][账号 {account_label}] 下次查询: {next_query_time.strftime('%Y-%m-%d %H:%M:%S')}[/dim]"
                        )
                ...
                console.print(
                    f"[dim][账号 {account_label}] 等待 {interval} 分钟...[/dim]"
                )
                await asyncio.sleep(interval * 60)
...
async def _run_watch_balance_async(
    ...
    if not exchange_name:
        raise ValueError(
            f"不支持的交易所: {exchange}，支持的交易所: {', '.join(exchange_name_map.keys())}"
        )
...
async def _run_generic_watch_balance_async(
    ...
    exchange_instance = create_exchange(
        exchange_type, api_key, api_secret, exchange_name, passphrase=passphrase
    )
    db_manager = DatabaseManager(database_url=database_url)
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    console.print(f"[red][账号 {account_label}] 查询失败:[/red] {e}")
                    logger.error(
                        "账号 %s watch-balance query error: %s", account_label, e
                    )
                    if debug:
                        console.print_exception()
...
async def _run_binance_watch_account_async(
    ...
    if enable_lark:
        console.print(
            "[yellow]提示:[/yellow] Binance 模式暂不支持 Lark 告警，选项已忽略。"
        )
    if enable_metrics:
        console.print(
            "[yellow]提示:[/yellow] Binance 模式暂不支持指标评估，选项已忽略。"
        )
    ...
    async def fetch_and_display(iteration_num: int):
        ...
        try:
            ...
            if spot_balances:
                ...
                console.print(
                    f"[green]✓[/green] [账号 {account_label}] 余额数据 (spot) 已保存到 [cyan]binance_account_snapshot[/cyan]"
                )
                # 更新 Prometheus 指标
                update_balance_metrics(
                    "binance",
                ...
                console.print(
                    f"[yellow][账号 {account_label}] Binance 现货账户余额为空[/yellow]"
                )
        except Exception as exc:
        ...
        try:
            ...
            if perp_balances:
                ...
                console.print(
                    f"[green]✓[/green] [账号 {account_label}] 余额数据 (perp) 已保存到 [cyan]binance_account_snapshot[/cyan]"
                )
                # 更新 Prometheus 指标
                update_balance_metrics(
                    "binance",
                ...
                console.print(
                    f"[yellow][账号 {account_label}] Binance 合约账户余额为空[/yellow]"
                )
        except Exception as exc:
        ...
        try:
            ...
            if positions:
                ...
                console.print(
                    f"[green]✓[/green] [账号 {account_label}] 仓位数据 (perp) 已保存到 [cyan]binance_position_snapshot[/cyan]"
                )
                # 更新 Prometheus 指标
                update_position_metrics(
                    "binance",
                ...
                console.print(
                    f"[yellow][账号 {account_label}] Binance 当前无持仓[/yellow]"
                )
        except Exception as exc:
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"Failed to update active orders metrics: {metric_error}",
                    exc_info=True,
                )
        except Exception as exc:
...
def watch_account(
    ...
    debug: bool = typer.Option(False, "--debug", help="启用调试模式"),
    enable_lark: bool = typer.Option(
        False, "--enable-lark", help="启用 Lark 告警推送（需配置 webhook）"
    ),
    interval_minutes: int = typer.Option(
        10, "--interval", "-i", help="查询间隔（分钟），默认 10 分钟"
    ),
    lark_webhook: Optional[str] = typer.Option(
        None,
        "--lark-webhook",
    ...
    lark_secret: Optional[str] = typer.Option(
        None, "--lark-secret", help="Lark 机器人签名密钥（可选；未提供则不做签名）"
    ),
    ...
    enable_metrics: bool = typer.Option(
        True, "--enable-metrics/--disable-metrics", help="是否启用指标评估（默认启用）"
    ),
    account_id: Optional[str] = typer.Option(
        None,
        "--account-id",
    ...
    try:
        ...
        def resolve_credentials(
            target_exchange: ExchangeName, key: Optional[str], secret: Optional[str]
        ) -> tuple[str, str]:
            env_prefix = target_exchange.value.upper()
            final_key = key or os.getenv(f"{env_prefix}_API_KEY", "")
            final_secret = secret or os.getenv(f"{env_prefix}_API_SECRET", "")
        ...
        if accounts or all_accounts:
            ...
            requested_ids = [
                acc_id.strip()
                for acc_id in (accounts.split(",") if accounts else [])
                if acc_id.strip()
            ]
            if all_accounts:
                requested_ids = [
            ...
            async def run_multi_account_watch():
                ...
                for acc in account_configs:
                    ...
                    if acc_exchange == ExchangeName.XT:
                        ...
                        console.print(
                            f"[yellow]警告:[/yellow] 账号 {acc.account_id} 的交易所 {acc_exchange.value} 暂不支持 watch-account，跳过"
                        )
                        continue
                ...
                if not tasks:
                    console.print(
                        "[red]错误:[/red] 选择的账号交易所暂未支持 watch-account 功能"
                    )
                    return
                ...
                try:
                    ...
                    for idx, result in enumerate(results):
                        if isinstance(result, Exception):
                            console.print(
                                f"[red]账号 {account_configs[idx].account_id} 监控任务异常:[/red] {result}"
                            )
                            logger.error(
                                "账号 %s watch-account 任务异常: %s",
                                account_configs[idx].account_id,
                                result,
                            )
                            if debug:
                                console.print_exception()
        ...
        final_api_key, final_api_secret = resolve_credentials(
        ...
        console.print(
            f"[red]错误:[/red] 交易所 '{exchange.value}' 暂不支持 watch-account 功能"
        )
        raise typer.Exit(code=1)
        ...
        try:
            ...
            try:
                ...
                if spot_balances:
                    ...
                    console.print(
                        f"[yellow][账号 {account_label}] XT 现货账户余额为空[/yellow]\n"
                    )
            except Exception as e:
                record_balance_query_status(
                    exchange_label,
                ...
                console.print(
                    f"[red][账号 {account_label}] 获取现货余额失败:[/red] {e}\n"
                )
                if debug:
                    console.print_exception()
            ...
            try:
                ...
                if perp_balances:
                    ...
                    for currency, balance_info in perp_balances.items():
                        ...
                        balances_data[currency] = {
                            ...
                            "unrealized_pnl": balance_info.get(
                                "unrealized_pnl", Decimal("0")
                            ),
                            "realized_pnl": balance_info.get(
                                "realized_pnl", Decimal("0")
                            ),
                            "equity": balance_info.get("equity", Decimal("0")),
                            "margin": balance_info.get("margin", Decimal("0")),
                            "margin_ratio": balance_info.get(
                                "margin_ratio", Decimal("0")
                            ),
                            # 保证金占用率计算所需字段
                            "openOrderMarginFrozen": balance_info.get(
                                "openOrderMarginFrozen",
                                balance_info.get("frozen", Decimal("0")),
                            ),
                            "isolatedMargin": balance_info.get(
                                "isolatedMargin", Decimal("0")
                            ),
                            "crossedMargin": balance_info.get(
                                "crossedMargin", Decimal("0")
                            ),
                            "totalAmount": balance_info.get(
                                "totalAmount", Decimal("0")
                            ),
                            "walletBalance": balance_info.get(
                                "walletBalance", Decimal("0")
                            ),
                            "marginBalance": balance_info.get(
                    ...
                    console.print(
                        f"[yellow][账号 {account_label}] XT 合约账户余额为空[/yellow]\n"
                    )
            except Exception as e:
                record_balance_query_status(
                    exchange_label,
                ...
                console.print(
                    f"[red][账号 {account_label}] 获取合约余额失败:[/red] {e}\n"
                )
                if debug:
                    console.print_exception()
                ...
                if positions:
                    ...
                    for pos in positions:
                        if hasattr(pos, "symbol"):
                            pos_symbol = pos.symbol
                            side = getattr(
                                pos, "side", getattr(pos, "position_side", "")
                            )
                            quantity = getattr(pos, "quantity", Decimal("0"))
                            entry_price = getattr(pos, "entry_price", Decimal("0"))
                            mark_price = getattr(pos, "mark_price", Decimal("0"))
                            ...
                            liquidation_price = getattr(
                                pos, "liquidation_price", Decimal("0")
                            )
                            leverage = getattr(pos, "leverage", "")
                            maintenance_margin = getattr(
                                pos, "maintenance_margin", Decimal("0")
                            )
                        else:
                            pos_symbol = pos.get("symbol", "")
                            side = pos.get("positionSide") or pos.get("side", "")
                        ...
                        pos_dict = {
                            ...
                            "breakPrice": str(liquidation_price),
                            "isolatedMargin": str(
                                getattr(pos, "margin", Decimal("0"))
                                if hasattr(pos, "margin")
                                else pos.get("isolatedMargin", "0")
                            ),
                    ...
                    if positions_data:
                        ...
                        console.print(
                            f"[yellow][账号 {account_label}] XT 当前无持仓[/yellow]\n"
                        )
                else:
                    console.print(
                        f"[yellow][账号 {account_label}] XT 当前无持仓[/yellow]\n"
            ...
            console.print(
                f"[dim][账号 {account_label}] 等待 {interval_minutes} 分钟...[/dim]\n"
            )
...
async def _run_binance_watch_positions_async(
    ...
    async def fetch_positions(iteration_num: int):
        ...
        for pos in positions:
            ...
            unrealized = Decimal(str(pos.get("unRealizedProfit", "0")))
            leverage = pos.get("leverage", "1")
            side = pos.get("positionSide") or (
                "LONG" if qty > 0 else "SHORT" if qty < 0 else "FLAT"
            )
    ...
    async def watch_loop():
        ...
        try:
            ...
            try:
                console.print(
                    f"[cyan]正在检查/创建数据库表（账号 {account_label}）...[/cyan]"
                )
                await db_manager.create_tables()
...
def watch_positions(
    ...
    interval: int = typer.Option(
        1, "--interval", "-i", help="查询间隔（分钟），默认1分钟"
    ),
    api_key: Optional[str] = typer.Option(
        None, "--api-key", help="API 密钥（覆盖环境变量）"
    ),
    ...
    accounts: Optional[str] = typer.Option(
        None, "--accounts", help="多个账号ID（逗号分隔），需要配合 --config 使用"
    ),
    all_accounts: bool = typer.Option(
        False,
        "--all-accounts",
    ...
    try:
            ...
            async def run_multi_account_watch():
                ...
                for acc in account_configs:
                    try:
                        acc_exchange = ExchangeName(acc.exchange.lower())
                        console.print(
                            f"[dim]调试: 账号 {acc.account_id} 交易所识别为: {acc_exchange.value}[/dim]"
                        )
                    except ValueError:
                        console.print(
                            f"[yellow]警告:[/yellow] 账号 {acc.account_id} 使用未支持的交易所 {acc.exchange}，跳过"
                    ...
                    if acc_exchange == ExchangeName.XT:
                        console.print(
                            f"[dim]调试: 账号 {acc.account_id} 路由到 XT 实现[/dim]"
                        )
                        task = asyncio.create_task(
                        ...
                        console.print(
                            f"[dim]调试: 账号 {acc.account_id} 路由到 Binance 实现[/dim]"
                        )
                        task = asyncio.create_task(
                        ...
                        console.print(
                            f"[yellow]警告:[/yellow] 账号 {acc.account_id} 的交易所 {acc_exchange.value} 暂不支持 watch-positions，跳过"
                        )
                        continue
                ...
                if not tasks:
                    console.print(
                        "[red]错误:[/red] 选择的账号交易所暂未支持 watch-positions 功能"
                    )
                    return
        ...
        if exchange == ExchangeName.XT:
            if not final_api_key or not final_api_secret:
                ...
                console.print("\n请设置环境变量或使用命令行参数:")
                console.print(
                    "  环境变量: export XT_API_KEY=your_key && export XT_API_SECRET=your_secret"
                )
                console.print("  命令行:   --api-key YOUR_KEY --api-secret YOUR_SECRET")
                console.print(
                    "  配置文件: --config config/accounts.json --account-id account_001"
                )
                raise typer.Exit(code=1)
        ...
        if exchange == ExchangeName.BINANCE:
            if not final_api_key or not final_api_secret:
                ...
                console.print("\n请设置环境变量或使用命令行参数:")
                console.print(
                    "  环境变量: export BINANCE_API_KEY=your_key && export BINANCE_API_SECRET=your_secret"
                )
                console.print("  命令行:   --api-key YOUR_KEY --api-secret YOUR_SECRET")
                console.print(
                    "  配置文件: --config config/accounts.json --account-id binance_main"
                )
                raise typer.Exit(code=1)
        ...
        console.print(
            f"[red]错误:[/red] 交易所 '{exchange.value}' 暂不支持 watch-positions 功能"
        )
        raise typer.Exit(code=1)
    ...
    try:
        ...
        if exchange == ExchangeName.XT:
            ...
            if accounts:
                ...
                if not config_path:
                    console.print(
                        "[red]错误:[/red] --all-accounts 需要配合 --config 使用"
                    )
                    raise typer.Exit(code=1)
                try:
                    from tri_arb.config.account_manager import AccountManager
            ...
            if config_path and account_id:
                try:
                    ...
                    console.print(
                        f"[yellow]警告:[/yellow] 读取配置文件失败: {e}，使用命令行参数或环境变量"
                    )
        ...
        console.print(
            f"[cyan]开始监控 {exchange.value.upper()} {exchange_type.value.upper()} 账户余额[/cyan]"
        )
        console.print(f"[cyan]查询间隔: {interval} 分钟[/cyan]")
        ...
        async def watch_loop():
            ...
            try:
                ...
                while True:
                    ...
                    console.print(f"\n{'='*60}")
                    console.print(
                        f"[bold]第 {iteration} 次查询 - {current_time}[/bold]"
                    )
                    console.print(f"{'='*60}\n")
                    ...
                    try:
                        ...
                        if (
                            ...
                            and exchange_type == ExchangeType.PERP
                            and balance_data
                        ):
                            filtered = {}
                                ...
                                for currency, data in balance_data.items():
                                    ...
                                    async with db_manager.session() as session:
                                        ...
                                        if record is not None:
                                            ...
                                            acc_label = (
                                                account_id or account_name or "默认账号"
                                            )
                                            console.print(
                                                f"[green]✓[/green] [账号 {acc_label}] 余额数据 ({exchange_type.value}) 已保存到 [cyan]{table_name}[/cyan]: {currency.upper()}"
                                            )
                                # 提交由 session ctx 管理
                            except Exception as save_exc:
...
def watch_account(
    ...
    try:
        ...
        if exchange != ExchangeName.XT:
            console.print(
                f"[red]错误:[/red] 交易所 '{exchange.value}' 暂时不支持 watch-account 功能"
            )
            console.print("\n目前仅支持以下交易所:")
                ...
                async def run_multi_account_watch():
                    ...
                    for acc_config in account_configs:
                        try:
                            ...
                            console.print(
                                f"[yellow]警告:[/yellow] 账号 {acc_config.account_id} 使用不支持的交易所 {acc_config.exchange}，跳过"
                            )
                            continue
                        ...
                        if acc_exchange == ExchangeName.XT:
                            task = asyncio.create_task(
                                _run_xt_watch_account_async(
                                    ...
                                    enable_lark=enable_lark,
                                    lark_webhook=(
                                        acc_config.lark_webhook if enable_lark else None
                                    ),
                                    lark_secret=(
                                        acc_config.lark_secret if enable_lark else None
                                    ),
                                    metrics_config=metrics_config,
                                    enable_metrics=enable_metrics,
                                    account_id=acc_config.account_id,
                            ...
                            console.print(
                                f"[yellow]警告:[/yellow] 账号 {acc_config.account_id} 的交易所 {acc_exchange.value} 暂不支持 watch-account，已跳过"
                            )
                            continue
                    ...
                    if not tasks:
                        console.print(
                            "[red]错误:[/red] 选择的账号交易所暂不支持 watch-account 功能"
                        )
                        return
            ...
            try:
                ...
                if account_config:
                    ...
                    try:
                        ...
                        console.print(
                            f"[yellow]警告:[/yellow] 账号 {account_id} 配置的交易所 {account_config.exchange} 无效，保持命令行参数"
                        )
        ...
        database_url = (
            account_manager.global_settings.get("database_url")
            if account_manager
            else None
        )
...
def watch_positions(
    ...
    try:
                    ...
                    async def run_multi_account_watch():
                        ...
                        for acc_config in account_configs:
                            try:
                                acc_exchange = ExchangeName(acc_config.exchange.lower())
                                console.print(
                                    f"[dim]调试: 账号 {acc_config.account_id} 交易所识别为: {acc_exchange.value}[/dim]"
                                )
                            except ValueError:
                                console.print(
                                    f"[yellow]警告:[/yellow] 账号 {acc_config.account_id} 使用未支持的交易所 {acc_config.exchange}，跳过"
                                )
                                continue

                            if acc_exchange == ExchangeName.XT:
                                console.print(
                                    f"[dim]调试: 账号 {acc_config.account_id} 路由到 XT 实现[/dim]"
                                )
                                task = asyncio.create_task(
                                    _run_xt_watch_positions_async(
                                        interval=interval,
                                        ...
                                        debug=debug,
                                        lark_webhook=(
                                            acc_config.lark_webhook
                                            if enable_lark
                                            else None
                                        ),
                                        lark_secret=(
                                            acc_config.lark_secret
                                            if enable_lark
                                            else None
                                        ),
                                        account_id=acc_config.account_id,
                                ...
                                console.print(
                                    f"[dim]调试: 账号 {acc_config.account_id} 路由到 Binance 实现[/dim]"
                                )
                                task = asyncio.create_task(
                                    _run_binance_watch_positions_async(
                                        interval=interval,
                                ...
                                console.print(
                                    f"[yellow]警告:[/yellow] 账号 {acc_config.account_id} 的交易所 {acc_exchange.value} 暂不支持 watch-positions，跳过"
                                )
                                continue
                        ...
                        try:
                            ...
                            for i, result in enumerate(results):
                                if isinstance(result, Exception):
                                    console.print(
                                        f"[red]账号 {account_configs[i].account_id} 监控任务异常:[/red] {result}"
                                    )
                                    logger.error(
                                        f"账号 {account_configs[i].account_id} 监控任务异常",
                                        exc_info=result,
                                    )
                                    if debug:
                                        console.print_exception()
        ...
        console.print(
            f"[cyan]开始监控 {exchange.value.upper()} 永续合约{symbol_text}持仓[/cyan]"
        )
        console.print(f"[cyan]查询间隔: {interval} 分钟[/cyan]")
        console.print(f"[yellow]按 Ctrl+C 停止监控[/yellow]\n")
                ...
                while True:
                    ...
                    try:
                        ...
                        if not positions_data:
                            if symbol:
                                console.print(
                                    f"[yellow]未发现 {symbol} 的持仓[/yellow]"
                                )
                            else:
                                console.print("[yellow]未发现持仓[/yellow]")
                        else:
                            ...
                            if output == "json":
                                ...
                                format_positions_table(
                                    positions_data, exchange_instance
                                )
                        ...
                        console.print(
                            f"[dim]下次查询: {next_query_time.strftime('%Y-%m-%d %H:%M:%S')}[/dim]"
                        )
...
def watch_orders(
    ...
    try:
        ...
        symbol_text = f"的 {symbol} " if symbol else ""
        console.print(
            f"[cyan]开始监控 {exchange.value.upper()} 永续合约{symbol_text}挂单[/cyan]"
        )
        console.print(f"[cyan]查询间隔: {interval} 分钟[/cyan]")
        console.print(f"[yellow]按 Ctrl+C 停止监控[/yellow]\n")
                ...
                while True:
                    ...
                    try:
                        ...
                        if not orders_data:
                            if symbol:
                                console.print(
                                    f"[yellow]未发现 {symbol} 的挂单[/yellow]"
                                )
                            else:
                                console.print("[yellow]未发现挂单[/yellow]")
                        else:
...
async def _run_watch_orders_async(
    ...
    exchange_instance = create_exchange(
        ExchangeType.PERP, api_key, api_secret, exchange, passphrase=passphrase
    )
        ...
        while True:
            ...
            try:
                ...
                if not orders_data:
                    ...
                    sell_orders = total_orders - buy_orders
                    console.print(
                        f"[dim][账号 {account_label}] 统计: 共 {total_orders} 个挂单 (买单: {buy_orders}, 卖单: {sell_orders})[/dim]"
                    )
...
def watch_all(
    config_path: str = typer.Option(
        "config/accounts.json", "--config", "-c", help="账号配置文件路径（JSON格式）"
    ),
    accounts: Optional[str] = typer.Option(
        None,
        "--accounts",
    ...
    database_url: Optional[str] = typer.Option(
        None, "--database-url", help="数据库连接URL（覆盖配置文件和环境变量）"
    ),
    debug: bool = typer.Option(False, "--debug", help="启用调试模式"),
):
    ...
    db_url = (
        database_url
        or account_manager.global_settings.get("database_url")
        or os.getenv("DATABASE_URL")
    )
    if not db_url:
        console.print("[red]错误:[/red] 未指定数据库URL")
        console.print(
            "请通过 --database-url 参数、配置文件 global_settings 或 DATABASE_URL 环境变量指定"
        )
        raise typer.Exit(code=1)
    ...
    if requested_ids is not None:
        enabled_accounts = [
            acc for acc in enabled_accounts if acc.account_id in requested_ids
        ]
        missing = set(requested_ids) - {acc.account_id for acc in enabled_accounts}
        if missing:
            console.print(
                f"[yellow]警告:[/yellow] 下列账号未启用或不存在: {', '.join(sorted(missing))}"
            )
    ...
    console.print(
        f"[cyan]数据库: {db_url.split('@')[-1] if '@' in db_url else db_url}[/cyan]"
    )
    ...
    for acc in enabled_accounts:
        ...
        if watch_tasks:
            ...
            if (
                balance_config.get("enabled", True)
                if isinstance(balance_config, dict)
                else balance_config
            ):
                tasks_str.append("balance")
            if (
                account_config.get("enabled", True)
                if isinstance(account_config, dict)
                else account_config
            ):
                tasks_str.append("account")
            if (
                positions_config.get("enabled", True)
                if isinstance(positions_config, dict)
                else positions_config
            ):
                tasks_str.append("positions")
            if (
                orders_config.get("enabled", True)
                if isinstance(orders_config, dict)
                else orders_config
            ):
                tasks_str.append("orders")
            console.print(
                f"  - {acc.account_id}: {acc.name} [{acc.exchange.upper()}] 任务: {', '.join(tasks_str) if tasks_str else '无'}"
            )
        else:
            console.print(
                f"  - {acc.account_id}: {acc.name} [{acc.exchange.upper()}] 任务: balance, account, positions (默认)"
            )
    ...
    async def run_all_watch_tasks():
        ...
        for acc in enabled_accounts:
            ...
            if isinstance(orders_config, bool):
                orders_config = {
                    "enabled": orders_config,
                    "interval": watch_tasks.get("interval", 5),
                }
            ...
            if balance_config.get("enabled", True):
                try:
                    ...
                    exchange_type_enum = (
                        ExchangeType.PERP
                        if exchange_type_str.lower() == "perp"
                        else ExchangeType.SPOT
                    )
                    ...
                    console.print(
                        f"[red]账号 {account_label} watch-balance 启动失败:[/red] {e}"
                    )
                    if debug:
                        console.print_exception()
            ...
            if account_config.get("enabled", True):
                try:
                    ...
                    if acc_exchange == ExchangeName.XT:
                        ...
                        console.print(
                            f"[yellow]警告:[/yellow] 账号 {account_label} 的交易所 {acc_exchange.value} 暂不支持 watch-account，跳过"
                        )
                        continue
                    ...
                    console.print(
                        f"[red]账号 {account_label} watch-account 启动失败:[/red] {e}"
                    )
                    if debug:
                        console.print_exception()
            ...
            if positions_config.get("enabled", True):
                try:
                    ...
                    if acc_exchange == ExchangeName.XT:
                        ...
                        console.print(
                            f"[yellow]警告:[/yellow] 账号 {account_label} 的交易所 {acc_exchange.value} 暂不支持 watch-positions，跳过"
                        )
                        continue
                    ...
                    console.print(
                        f"[red]账号 {account_label} watch-positions 启动失败:[/red] {e}"
                    )
                    if debug:
                        console.print_exception()
            ...
            if orders_config.get("enabled", False):
                try:
                    ...
                    console.print(
                        f"[red]账号 {account_label} watch-orders 启动失败:[/red] {e}"
                    )
                    if debug:
                        console.print_exception()
