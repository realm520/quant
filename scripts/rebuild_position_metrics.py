...
async def _calculate_metrics_for_time(
    ...
    """
    # 获取该时间点所在日期的零点快照
    today_midnight = datetime.combine(target_time.date(), datetime.min.time()).replace(
        tzinfo=None
    )
    ...
    if symbol:
        all_today_snapshots_query = all_today_snapshots_query.where(
            PositionMetrics.symbol == symbol
        )
    ...
    if today_date in today_series_result:
        for sym, metrics in today_series_result[today_date].items():
            ...
            close_prices = await calc._get_close_prices(
                today_midnight, target_time, sym
            )
    ...
    for sym, m in today_metrics.items():
        ...
        if midnight_snapshot:
            ...
            cumulative_realized_pnl_at_midnight = (
                midnight_snapshot.cumulative_realized_pnl or Decimal("0")
            )
        ...
        cumulative_realized_pnl_now = (
            cumulative_realized_pnl_at_midnight + today_realized_pnl
        )
        cumulative_pnl = cumulative_realized_pnl_now + today_unrealized_pnl

        open_left_long_qty_from_snapshot = (
            midnight_snapshot.left_long_qty if midnight_snapshot else Decimal("0")
        )
        open_left_short_qty_from_snapshot = (
            midnight_snapshot.left_short_qty if midnight_snapshot else Decimal("0")
        )
        open_left_long_value_from_snapshot = (
            midnight_snapshot.left_long_value if midnight_snapshot else Decimal("0")
        )
        open_left_short_value_from_snapshot = (
            midnight_snapshot.left_short_value if midnight_snapshot else Decimal("0")
        )
...
async def rebuild_all_metrics(
    ...
    async with db_manager.session() as session:
        try:
            ...
            # 2. 创建合约乘数服务（如果需要）
            from tri_arb.services.contract_multiplier_service import (
                ContractMultiplierService,
            )
            ...
            if exchange == "binance":
                trade_time_query = trade_time_query.where(
            ...
            if (
                not trade_time_range
                or not trade_time_range.min_time
                or not trade_time_range.max_time
            ):
            ...
            min_time = datetime.combine(min_date, datetime.min.time()).replace(
                tzinfo=None
            )
            ...
            existing_times_query = (
                select(PositionMetrics.timestamp, PositionMetrics.symbol)
                .where(PositionMetrics.account_id == account_id)
                .where(PositionMetrics.exchange == exchange)
            )
            if symbol:
                existing_times_query = existing_times_query.where(
                    PositionMetrics.symbol == symbol
                )
            ...
            existing_times_set = {
                (row[0], row[1]) for row in existing_times_result.all()
            }
            ...
            if current_time.minute % 5 != 0:
                current_time = current_time.replace(
                    minute=(current_time.minute // 5 + 1) * 5, second=0, microsecond=0
                )
            ...
            while current_time <= max_time:
                ...
                try:
                    ...
                    if calculated_count % 100 == 0:
                        await session.commit()
                        print(
                            f"已计算 {calculated_count} 个时间点，跳过 {skipped_count} 个已存在的时间点..."
                        )
...
async def main():
    ...
    parser.add_argument(
        "--exchange", required=True, help="交易所名称（如 xt, binance）"
    )
    parser.add_argument(
        "--symbol", default=None, help="交易对（可选），不指定则处理所有交易对"
    )
    parser.add_argument(
        "--database-url",
        default=None,
