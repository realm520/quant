...
class OrderUpdate(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "exchange", "order_id", "event_time", name="uq_order_update_event"
        ),
    )
...
class ConnectionStatus(Base):
    ...
    # 最后处理的事件时间戳（用于数据补全）
    last_order_event_time = Column(
        DateTime, nullable=True, index=True
    )  # 最后处理的订单事件时间
    last_trade_event_time = Column(
        DateTime, nullable=True, index=True
    )  # 最后处理的成交事件时间
    last_account_event_time = Column(
        DateTime, nullable=True, index=True
    )  # 最后处理的账户事件时间
    ...
    __table_args__ = (Index("idx_exchange_connected", "exchange", "is_connected"),)
