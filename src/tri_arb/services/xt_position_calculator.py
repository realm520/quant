...
class XTPositionCalculator:
    ...
    async def calculate_pre_position_metrics(
        ...
        if use_websocket:
            # 使用 WebSocket 数据（推荐）
            return await self.calculate_pre_position_metrics_from_websocket(target_date, hours_back)
        else:
            # 使用 REST API 持仓快照数据
            yesterday_positions = await self.get_yesterday_positions(target_date, hours_back)
        ...
        for symbol, sides in yesterday_positions.items():
            ...
            if "SHORT" in sides:
                ...
                if short_data["notional"] > 0:
                    ...
                    pre_short_value += short_data["quantity"] * short_data["entry_price"]
    ...
    async def calculate_position_change(
        ...
        """
        current_metrics = await self.calculate_current_position_metrics(current_positions)
        pre_metrics = await self.calculate_pre_position_metrics(target_date, hours_back)
        
        changes = {
            "long_qty_change": current_metrics["long_qty"] - pre_metrics["pre_long_qty"],
            "short_qty_change": current_metrics["short_qty"] - pre_metrics["pre_short_qty"],
            "long_value_change": current_metrics["long_value"] - pre_metrics["pre_long_value"],
            "short_value_change": current_metrics["short_value"] - pre_metrics["pre_short_value"],
        }
    ...
    async def calculate_pre_position_metrics_from_websocket(
        ...
        Returns:
            ...
            - pre_short_value: 昨日空头持仓市值（所有symbol的空头持仓市值之和）
        """
        yesterday_positions = await self.get_yesterday_positions_from_websocket(target_date, hours_back)
