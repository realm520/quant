...
class BtmxTradeHistory(Base):
    __tablename__ = 'btmx_trade_history'
    ...
    def __repr__(self):
        return f"<BtmxTradeHistory(coid='{self.coid}'>"
...
engine = create_engine('sqlite:///grid_trade.db', echo=True, poolclass=SingletonThreadPool, connect_args={'check_same_thread': False})
...
if __name__ == '__main__':
    session = Session()
    session.add(BtmxTradeHistory(coid="12121212", baseAsset="BTMX"))
    session.add(BtmxCompletedPosition(symbol='BTMX-USDT', openOrderId="12121212", closeOrderId="12121212"))
    session.commit()
