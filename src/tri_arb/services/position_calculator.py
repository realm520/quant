...
class PositionCalculator:
    ...
    def __init__(
        ...
        self.contract_multiplier_getter = contract_multiplier_getter or (
            lambda s: Decimal("1")
        )
    ...
    def _get_contract_multiplier(self, symbol: str) -> Decimal:
        ...
        try:
            ...
            logger.warning(
                f"Failed to get contract multiplier for {symbol}, using default 1: {e}"
            )
            return Decimal("1")
    ...
    async def calculate_yesterday_end_left_qty_value(
        ...
        if initial_positions_dict is not None:
            for s, pos_data in initial_positions_dict.items():
                by_symbol[s] = {
                    ...
                    "initial_short_qty": pos_data.get(
                        "initial_short_qty", Decimal("0")
                    ),
                    "initial_long_value": pos_data.get(
                        "initial_long_value", Decimal("0")
                    ),
                    "initial_short_value": pos_data.get(
        ...
        for s, data in by_symbol.items():
            ...
            if (
                buy_volume == 0
                and sell_volume == 0
                and initial_long_qty == 0
                and initial_short_qty == 0
            ):
                result_dict[s] = {
            ...
            # 计算剩余市值
            left_long_value = (
                left_long_qty * avg_buy_prz if avg_buy_prz > 0 else Decimal("0")
            )
            left_short_value = (
                left_short_qty * avg_sell_prz if avg_sell_prz > 0 else Decimal("0")
            )
    ...
    async def calculate_position_from_trades(
        ...
        # 2. 统计区间内所有成交记录
        buy_volume, sell_volume = await self._calculate_trade_volumes(
        ...
        buy_trade_value, sell_trade_value = await self._calculate_trade_values(
            start_time, end_time, symbol
        )
    ...
    async def calculate_cumulative_pnl(
        ...
        while current_date <= end_date_only:
            ...
            day_metrics_by_symbol = await self.calculate_positions_by_symbol(
                start_time=day_start, end_time=day_end, symbol=symbol
            )
        ...
        current_metrics_by_symbol = await self.calculate_positions_by_symbol(
            start_time=datetime(
                end_date_only.year, end_date_only.month, end_date_only.day
            ),
            end_time=end_date,
            symbol=symbol,
        )
    ...
    async def _calculate_trade_volumes(
        ...
        for trade_symbol, side, total_quantity_contracts in rows:
            # 获取合约乘数并转换为币的数量
            contract_multiplier = self._get_contract_multiplier(trade_symbol)
            total_quantity_coins = (
                total_quantity_contracts or Decimal("0")
            ) * contract_multiplier
    ...
    async def _get_close_prices(
        ...
        subquery = (
            select(self.TradeModel.symbol, func.max(time_column).label("max_time"))
            .where(time_column >= start_time)
            .where(time_column < end_time)
        )
        ...
        query_all = (
            select(self.TradeModel.symbol, self.TradeModel.price, time_column)
            .where(time_column >= start_time)
            .where(time_column < end_time)
        )
        ...
        for trade_date in sorted_dates:
            ...
            for symbol in all_symbols:
                ...
                open_left_long_value = (
                    open_left_long_qty * prev_avg_buy_prz
                    if prev_avg_buy_prz > 0
                    else Decimal("0")
                )
                open_left_short_value = (
                    open_left_short_qty * prev_avg_sell_prz
                    if prev_avg_sell_prz > 0
                    else Decimal("0")
                )
