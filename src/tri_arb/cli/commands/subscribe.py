...
def user_stream(
    exchange: ExchangeName = typer.Option(
        ..., "--exchange", "-x", help="交易所: binance, okx, gate, xt"
    ),
    ...
    passphrase: Optional[str] = typer.Option(
        None, "--passphrase", help="API Passphrase（OKX需要，覆盖环境变量）"
    ),
    database_url: Optional[str] = typer.Option(
        None, "--database-url", help="PostgreSQL连接URL（覆盖环境变量）"
    ),
    create_tables: bool = typer.Option(
        False, "--create-tables", help="自动创建数据库表"
    ),
    ...
    try:
        ...
        db_url = database_url or os.getenv(
            "DATABASE_URL", "postgresql+asyncpg://oliver@localhost:5432/trading"
        )
        ...
        if channels:
            ...
            if exchange == ExchangeName.BINANCE:
                ...
                if invalid:
                    console.print(
                        f"[red]错误:[/red] Binance不支持的频道: {', '.join(invalid)}"
                    )
                ...
                if invalid:
                    console.print(
                        f"[red]错误:[/red] OKX不支持的频道: {', '.join(invalid)}"
                    )
                    console.print(f"支持的频道: {', '.join(valid_channels)}")
                    raise typer.Exit(code=1)
            elif exchange == ExchangeName.GATE:
                ...
                if invalid:
                    console.print(
                        f"[red]错误:[/red] Gate.io不支持的频道: {', '.join(invalid)}"
                    )
                    console.print(f"支持的频道: {', '.join(valid_channels)}")
                    raise typer.Exit(code=1)
            elif exchange == ExchangeName.XT:
                ...
                if invalid:
                    console.print(
                        f"[red]错误:[/red] XT不支持的频道: {', '.join(invalid)}"
                    )
                    console.print(f"支持的频道: {', '.join(valid_channels)}")
                    raise typer.Exit(code=1)
        ...
        console.print(
            f"[cyan]数据库: {db_url.split('@')[-1] if '@' in db_url else 'localhost'}[/cyan]"
        )
        if channel_list:
        ...
        console.print(
            f"[cyan]数据同步: {'启用' if enable_data_sync else '禁用'}[/cyan]"
        )
        ...
        async def run_service():
            ...
            try:
                ...
                console.print(
                    f"[yellow]警告:[/yellow] 基础数据库表初始化失败: {init_exc}"
                )
...
def multi_account(
    ...
    database_url: Optional[str] = typer.Option(
        None, "--database-url", help="PostgreSQL连接URL（覆盖环境变量和配置文件）"
    ),
    create_tables: bool = typer.Option(
        False, "--create-tables", help="自动创建数据库表"
    ),
    ...
    if account_ids:
        requested_ids = [
            acc_id.strip() for acc_id in account_ids.split(",") if acc_id.strip()
        ]
    ...
    for acc in enabled_accounts:
        ...
        console.print(
            f"  - {acc.account_id}: {acc.name} [{exchange_name}] 频道: {channels_desc}"
        )
    ...
    async def prepare_tables_if_needed():
        ...
        try:
            ...
            logger.error(
                "Failed to create database tables for multi-account",
                exc_info=True,
    ...
    async def run_account_stream(acc_config):
        try:
            ...
            console.print(
                f"[yellow]警告:[/yellow] 账号 {acc_config.account_id} 的交易所 {acc_config.exchange} 暂不支持，跳过"
            )
            return
        ...
        if not key or not secret:
            console.print(
                f"[yellow]警告:[/yellow] 账号 {acc_config.account_id} 缺少 API 凭证，跳过"
            )
            return

        db_manager = DatabaseManager(database_url=db_url)
        enabled_channels = (
            [ch.strip().lower() for ch in acc_config.channels]
            if acc_config.channels
            else None
        )
        ...
        if acc_exchange == ExchangeName.BINANCE:
            ...
            if not passphrase:
                console.print(
                    f"[yellow]警告:[/yellow] 账号 {acc_config.account_id} 缺少 OKX passphrase，跳过"
                )
                await db_manager.close()
                return
            service = OKXUserStreamService(
        ...
        console.print(
            f"[cyan]账号 {acc_config.account_id} ({acc_config.name}) 开始订阅 - {acc_exchange.value.upper()}[/cyan]"
        )
        ...
        try:
            ...
            logger = logging.getLogger(__name__)
            logger.error(
                "account %s stream error", acc_config.account_id, exc_info=debug
            )
            if debug:
                console.print_exception()
        finally:
...
def okx_user_stream_shortcut(
    ...
    api_secret: Optional[str] = typer.Option(None, "--api-secret", help="API 密钥"),
    passphrase: Optional[str] = typer.Option(
        None, "--passphrase", help="API Passphrase"
    ),
    database_url: Optional[str] = typer.Option(
        None, "--database-url", help="数据库URL"
    ),
