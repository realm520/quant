...
async def example_query_data():
    ...
    async with db_manager.session() as session:
        ...
        for order in orders:
            print(
                f"  {order.symbol} - {order.side} - {order.order_status} - {order.event_time}"
            )
        ...
        stmt = (
            ...
            .where(func.date(TradeUpdate.transaction_time) == today)
        ...
        for stat in stats:
            print(
                f"  {stat.symbol}: {stat.count}笔, 数量: {stat.total_qty}, 手续费: {stat.total_fee}"
            )
        ...
        for update in updates:
            if update.asset:
                print(
                    f"  {update.asset} - 余额: {update.wallet_balance} - {update.event_time}"
                )
            elif update.symbol:
                print(
                    f"  {update.symbol} - 持仓: {update.position_amount} - {update.event_time}"
                )
