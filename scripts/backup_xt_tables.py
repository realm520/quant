...
def backup_tables():
    ...
    try:
        with conn.cursor() as cur:
            print("=" * 80)
            print(
                f"XT 表备份工具 (备份时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')})"
            )
            print("=" * 80)
            print()
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            for original_name in sorted(backup_groups.keys()):
                ...
                for backup_name, timestamp in sorted(
                    backup_groups[original_name], reverse=True
                ):
                    # 解析时间戳
                    try:
...
def main():
    ...
    parser.add_argument(
        ...
        help="操作: backup(备份), list(列出备份), restore(恢复)",
    )
    parser.add_argument("--backup-name", type=str, help="备份表名（用于 restore 操作）")
    parser.add_argument(
        "--restore-name", type=str, help="恢复后的表名（用于 restore 操作，可选）"
    )
