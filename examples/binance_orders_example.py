    ...
    try:
        ...
        if not orders:
            ...
            for order in orders:
                ...
                filled_pct = (
                    (executed_qty / orig_qty * 100) if orig_qty > 0 else Decimal("0")
                )
...
async def example_analyze_orders():
    ...
    try:
        ...
        for order_type, count in sorted(
            order_types.items(), key=lambda x: x[1], reverse=True
        ):
        ...
        for order in recent_orders:
            order_time = datetime.fromtimestamp(order["time"] / 1000)
            print(
                f"  {order['symbol']} - {order['side']} {order['type']} @ {order['price']} - {order_time.strftime('%H:%M:%S')}"
            )
