...
- API 权限：为需要的功能开启读取/交易/期货权限（XT/OKX/Binance）。
- 时间同步：服务器时间请与 NTP 同步，减少签名与时间相关错误（XT 偶发 403）。
- 数据库连接：`DATABASE_URL` 用户名请使用本机实际用户（macOS 常见为系统用户名）。
- 虚拟环境：避免直接在系统 Python 中安装，使用 `uv venv` 或 `python -m venv`（PEP 668）。
- 频率限制：遵守各交易所频率限制，必要时在命令中降低轮询频率或使用 `--debug` 观察提示。
- 日志与运维：生产建议开启日志轮转与备份，数据库定期维护索引，磁盘空间预警。
- 安全：妥善保管 API Key，不要将密钥提交到仓库，建议通过 `.env` + `load_env.sh` 加载。
