...
class TradingPath(BaseModel, frozen=True):
    ...
    start_currency: str = Field(
        ...,
        description="Starting currency (e.g., USDT)"
    )
...
class ArbitrageOpportunity(BaseModel):
    ...
    path: TradingPath = Field(
        ...,
        description="The arbitrage trading path"
    )
    ...
    recommended_amount: Decimal = Field(
        ...,
        gt=0,
        description="Recommended initial investment amount"
    )
    ...
    def validate_prices_length(cls, v: list[dict]) -> list[dict]:
        ...
        for price in v:
            ...
            if price["type"] not in ["buy", "sell"]:
                raise ValueError(f"Price type must be 'buy' or 'sell', got: {price['type']}")
