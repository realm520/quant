...
class OKXUserStreamService:
    ...
    async def _monitor_connection_health(self):
        ...
        logger.info("Starting connection health monitor (monitoring only, no forced reconnection)")
    ...
    async def save_order_update(self, data: dict):
        ...
        if saved_count > 0:
            logger.info("OKX order updates saved", saved=saved_count, duplicates=duplicate_count)
        elif duplicate_count > 0:
            logger.debug("All OKX order updates were duplicates", duplicates=duplicate_count)
    ...
    async def query_missing_data(self, symbols: list[str] | None = None):
        ...
        if not self.exchange.is_connected:
            logger.info("Exchange not connected, connecting now for order data recovery")
        ...
        for symbol in symbols:
            ...
            try:
                ...
                logger.error(f"Failed to query OKX order data for {symbol}", error=str(e), exc_info=True)
                continue
    ...
    async def _save_order_with_dedup(self, order_data: dict) -> bool:
        ...
        update_time = datetime.fromtimestamp(int(order_data.get("uTime", 0)) / 1000) if order_data.get("uTime") else datetime.utcnow()
        ...
        try:
            async with self.db_manager.session() as session:
                okx_order = OKXOrder(
                    ...
                    px=_safe_decimal(order_data.get("px")) if order_data.get("px") else None,
                    avg_px=_safe_decimal(order_data.get("avgPx")) if order_data.get("avgPx") else None,
                    acc_fill_sz=_safe_decimal(order_data.get("accFillSz")) if order_data.get("accFillSz") else None,
                    state=order_data.get("state"),
                    fee=_safe_decimal(order_data.get("fee")) if order_data.get("fee") else None,
                    fee_ccy=order_data.get("feeCcy"),
                    rebate=_safe_decimal(order_data.get("rebate")) if order_data.get("rebate") else None,
                    rebate_ccy=order_data.get("rebateCcy"),
                    c_time=datetime.fromtimestamp(_safe_float(order_data.get("cTime")) / 1000) if order_data.get("cTime") else None,
                    u_time=update_time,
    ...
    async def _recover_data_with_retry(self, max_retries: int = 3, retry_delay: int = 2):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                logger.info("OKX order data recovery completed successfully", attempt=attempt)
                return  # 成功，退出
                ...
                if attempt < max_retries:
                    logger.info(f"Retrying OKX order data recovery in {retry_delay} seconds...")
        ...
        try:
            ...
            async with ws_connect(
                ...
                if needs_recovery and disconnect_time:
                    ...
                    try:
                        await self._recover_data_with_retry(max_retries=3, retry_delay=2)
